# syntax=docker/dockerfile:1
ARG NODE_VERSION=23-bookworm
ARG DEBIAN_CODENAME=slim
ARG SOURCE_DIR=/home/jenkins

FROM node:${NODE_VERSION}-${DEBIAN_CODENAME} AS base
ARG SOURCE_DIR
WORKDIR "$SOURCE_DIR"
COPY . .

RUN corepack enable && \
  apt-get update -y && \
  apt-get install -y openssl


FROM base AS runtime-deps
RUN --mount=type=cache,id=pnpm,target=/pnpm/store pnpm install --prod --frozen-lockfile


FROM base AS builder
# Use the approach here to build faster
# https://turbo.build/repo/docs/guides/tools/docker
# Set the environment to production only for build
RUN --mount=type=cache,id=pnpm,target=/pnpm/store pnpm install --frozen-lockfile
RUN pnpm run build


FROM builder AS test
ARG SOURCE_DIR
WORKDIR "$SOURCE_DIR"
#RUN pnpm run test
#RUN pnpm run lint
RUN echo "nothing to do"

FROM base AS runtime

ARG SOURCE_DIR
WORKDIR "$SOURCE_DIR"
ENV NODE_ENV=production

RUN apt-get update -y && \
  apt-get install -y openssl && \
  addgroup --system --gid 1001 nodejs && \
  adduser --system --uid 1001 nextjs

# Set the correct permission for prerender cache
RUN mkdir -p api worker-kafka worker-grpc && \
  chown nextjs:nodejs api worker-kafka worker-grpc

EXPOSE 3000
ENV PORT=3000
ENV HOSTNAME="0.0.0.0"

ENV PRISMA_QUERY_ENGINE_LIBRARY "${SOURCE_DIR}/libquery_engine-debian-openssl-3.0.x.so.node"

COPY --from=builder --chown=nextjs:nodejs ["${SOURCE_DIR}/packages/database/generated/client/libquery_engine-debian-openssl-3.0.x.so.node", "."]

COPY --from=builder --chown=nextjs:nodejs ["${SOURCE_DIR}/packages/api/public", "./api/public"]
COPY --from=builder --chown=nextjs:nodejs ["${SOURCE_DIR}/packages/api/.next/standalone", "./api"]
COPY --from=builder --chown=nextjs:nodejs ["${SOURCE_DIR}/packages/api/.next/static", "./api/.next/static"]

COPY --from=builder --chown=nextjs:nodejs ["${SOURCE_DIR}/apps/worker-kafka/dist", "./worker-kafka"]
COPY --from=builder --chown=nextjs:nodejs ["${SOURCE_DIR}/apps/worker-grpc/dist", "./worker-grpc"]

CMD ["node ${ENTRYPOINT}"]
#ENTRYPOINT ["tail", "-f", "/dev/null"]

